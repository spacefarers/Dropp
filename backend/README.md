...
Flask-based backend service that issues pre-signed upload/download URLs for Vercel Blob storage, stores file metadata in MongoDB, and authenticates users through Firebase for the desktop applications.
...
- `POST /auth/firebase/session` – verifies a Firebase ID token and returns the token plus user metadata so clients can persist it locally.
- `GET /list/` – returns metadata for the authenticated user's files. Always include a Firebase ID token via `Authorization: Bearer <token>`.
